...
<div class="row">
    ...
    <div class="kt-portlet__head-actions">
       &nbsp;
       @if($role==2)
        <a href="{{ url('add_location')}}" class="btn btn-brand btn-elevate btn-icon-sm">
        ...
        <table class="table table-striped- table-bordered table-hover table-checkable" id="kt_table_1">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    @if($role==2)
                    <td><?php echo App\Helpers\Helper::get_mapped_user($loggedin_user, $row->id)?App\Helpers\Helper::get_mapped_user($loggedin_user, $row->id):"NA" ?>                        
                    </td>
                    <td>
                        <a class="btn btn-sm btn-clean btn-icon btn-icon-md" href="{{url('edit_location/'.$row->id)}}" title="Edit Location" >
                            <i class="fa fa-edit"></i>
                        </a>
                        @if($row->is_active==1)
                        <a href="{{url('change_location_status/'.$row->id.'/0')}}" title="Deactivate location" onclick="return confirm('This location will be deactivated. Are you sure to proceed?');">
                                <i class="fa fa-times"></i>
                              </a>
                        @else
                        <a href="{{url('change_location_status/'.$row->id.'/1')}}" title="Activate location">
