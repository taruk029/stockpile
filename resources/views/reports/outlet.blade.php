@extends('layouts.app')
@section('title', ' Outlet Form Report ')
@push('styles')
...
<div class="row">
    ...
    <form class="kt-form kt-form--label-right" method="post" action="{{url('outlet_report')}}">
    {{ csrf_field() }}
            ...
            <div class="form-group">
                ...
                <div class='input-group'>
                        <select class="form-control" id="trainer" name="trainer">
                            ...
                            @if($trainers)
                                @foreach($trainers as $rows)
                                <option value="{{$rows->id}}" <?php if(isset($_POST['trainer'])) { if($_POST['trainer']==$rows->id) echo "selected"; } ?> > {{ $rows->employee_name }} </option>
                            @endforeach
                            @endif
                        </select>
                    </div>
        ...
        <table class="table table-striped- table-bordered table-hover table-checkable" id="kt_table_1">
            <thead>
                <tr>
                    ...
                    <th>Outlet Code</th>
                    <th>Outlet Name</th>
                    <th>Outlet Productivity</th>
                    <th>Non Productivity Reason</th>
                </tr>
            </thead>
...
<script type="text/javascript">
    ...
    function s_excel()
    {
        ...
        var trainer = "";
        trainer = $("#trainer").val();
        window.open("{{ url('outlet_excel/?date=') }}"+date+"&trainer="+trainer);
    }
